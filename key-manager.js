/* ═══════════════════════════════════════════════════════════
   key-manager.js — v19 HASH+AES HEAVY
   NightOrbit CodeForge

   UPGRADE FROM V18 → V19:
   ✅ ADDED: File name HASH (SHA-256, dynamic, no decode)
   ✅ ADDED: Key 1 AES-256-CBC encrypted (was obfuscated)
   ✅ ADDED: generateHashedFileName() — dynamic per file
   ✅ ADDED: _generateFileHashFunction() — buried in function
   ✅ ADDED: encryptKey1AES() — AES-256-CBC for Key 1
   ✅ ADDED: decryptKey1AES() — AES-256-CBC decrypt
   ✅ PRESERVED: All v18 functions (NO REMOVAL)
   ✅ PRESERVED: Rate limiting
   ✅ PRESERVED: Audit logging
   ✅ PRESERVED: Device fingerprint
   ✅ PRESERVED: HMAC integrity
   ═══════════════════════════════════════════════════════════ */
...
var KEY_MANAGER = {
    ...
    /* ═══════════════════════════════════════════════════════
       ✅ NEW v19: DYNAMIC HASH FILE NAME
       Har file ke liye unique hash code — decode nahi hoga
       ═══════════════════════════════════════════════════════ */
    generateHashedFileName: function() {
        /* Random seed */
        ...
        /* Format: nx_<32chars>.js */
        var chars = 'abcdefghijklmnopqrstuvwxyz0123456789';
        var name = 'nx_';
        for (var i = 0; i < 32; i++) {
            var hexPair = hash.substring(i * 2, i * 2 + 2);
            var idx = parseInt(hexPair, 16) % chars.length;
            name += chars.charAt(idx);
        }
        name += '.js';
    ...
    /* ✅ NEW v19: File name hash buried in function chain */
    _generateFileHashFunction: function(fileName) {
        /* 6-layer function chain */
        var xorKey1 = this._secureRandomInt(251) + 1;
        var xorKey2 = this._secureRandomInt(251) + 1;
        
        /* Layer 1: XOR each char */
        var layer1 = '';
        for (var i = 0; i < fileName.length; i++) {
            layer1 += String.fromCharCode(fileName.charCodeAt(i) ^ xorKey1);
        }
        ...
        var chunks = [];
        for (var k = 0; k < layer3.length; k += 4) {
            chunks.push(layer3.substring(k, k + 4));
        }
        
        /* Layer 5: XOR chunks with second key */
        var chunkXor = [];
        for (var m = 0; m < chunks.length; m++) {
            var chunk = chunks[m];
            var xored = '';
            for (var n = 0; n < chunk.length; n++) {
                xored += String.fromCharCode((chunk.charCodeAt(n) ^ xorKey2) ^ (n % 127));
            }
            chunkXor.push(btoa(xored));
        }
        
        /* Layer 6: Build function chain */
        var code = '';
        code += 'var _fn = function() { return [';
        for (var p = 0; p < chunkXor.length; p++) {
            code += '"' + chunkXor[p] + '"';
            if (p < chunkXor.length - 1) code += ',';
        }
        code += '].join(""); };\n';
        code += 'var _fn2 = function(s) { return s.split("").reverse().join(""); };\n';
        code += 'var _fn3 = function(s, k) { var r = ""; for (var i = 0; i < s.length; i++) r += String.fromCharCode((s.charCodeAt(i) ^ k) ^ (i % 127)); return atob(r); };\n';
        code += 'var _fn4 = function(s, k) { var r = ""; for (var i = 0; i < s.length; i++) r += String.fromCharCode(s.charCodeAt(i) ^ k); return r; };\n';
        code += 'var _fn5 = function(arr, k) { var r = ""; for (var i = 0; i < arr.length; i++) r += _fn3(arr[i], k); return r; };\n';
        code += 'window._0xFNAME = _fn4(_fn2(_fn5(_fn().split("\\x01"), ' + xorKey2 + ')), ' + xorKey1 + ');\n';
    ...
    /* ═══════════════════════════════════════════════════════
       ✅ NEW v19: AES-256-CBC ENCRYPTED KEY 1
       Key 1 ko AES-256-CBC se encrypt karo
       ═══════════════════════════════════════════════════════ */
    encryptKey1AES: function(key1, masterSecret) {
        try {
            ...
            /* Format: v3:iv:ciphertext:hmac */
            return 'v3:' + iv.toString() + ':' + encrypted.toString() + ':' + hmac;
        } catch (e) {
            throw new Error('Key 1 AES encryption failed');
        }
    },

    decryptKey1AES: function(encryptedKey1, masterSecret) {
        try {
            ...
            /* Decrypt */
            var decrypted = CryptoJS.AES.decrypt(ciphertext, masterSecret, {
                iv: iv,
                mode: CryptoJS.mode.CBC,
                padding: CryptoJS.pad.Pkcs7
            ...
            throw new Error('Key 1 AES decryption failed');
        }
    },
    ...
    hasKey: function() {
        return !!(this._initialized && this._key);
    },
...
console.log('%c🔐 Key Manager v19 HASH+AES HEAVY loaded',
    'color:#00ff64;font-weight:bold;font-size:14px;');
console.log('%c⚡ SHA-256 File Name + AES-256-CBC Key 1 + HMAC',
    'color:#ffd700;font-size:11px;');
console.log('%c📊 Rate Limiting | Audit Logging | Device Fingerprint',
